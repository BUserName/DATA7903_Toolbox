...
drop materialized view if exists mv_all_vars;
create materialized view mv_all_vars as
...
---- GCS
    ...
    and valuenum>=3
    and hadm_id in (select distinct hadm_id from icustays_adults);


CREATE INDEX mv_all_vars_hadm_id_idx
  ON mimic3.mv_all_vars
  USING btree
  (hadm_id);
