...
drop materialized view if exists mv_adult_all_vars_tw;
create materialized view mv_adult_all_vars_tw as
select asv.subject_id, asv.hadm_id, asv.icustay_id, asv.itemcat, asv.itemid, asv.charttime, icu.intime, icu.outtime, asv.cvalue, asv.valuenum, asv.valueuom, 
  ...
  from mv_adult_all_vars asv,
  mv_icustays_adult icu
...
CREATE INDEX idx01_mv_adult_all_vars_tw
  ON mimic3.mv_adult_all_vars_tw
  USING btree
  (subject_id, hadm_id, itemcat, itemid);
CREATE INDEX idx02_mv_adult_all_vars_tw
  ON mimic3.mv_adult_all_vars_tw
  USING btree
  (subject_id);
CREATE INDEX idx03_mv_adult_all_vars_tw
  ON mimic3.mv_adult_all_vars_tw
  USING btree
  (hadm_id);
  CREATE INDEX idx04_mv_adult_all_vars_tw
  ON mimic3.mv_adult_all_vars_tw
  USING btree
  (itemid);
  CREATE INDEX idx05_mv_adult_all_vars_tw
  ON mimic3.mv_adult_all_vars_tw
  USING btree
  (itemcat);