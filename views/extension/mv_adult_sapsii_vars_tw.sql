...
drop materialized view if exists mv_adults_sapsii_vars_tw;
create materialized view mv_adults_sapsii_vars_tw as
select asv.subject_id, asv.hadm_id, asv.icustay_id, asv.itemcat, asv.itemid, asv.charttime, icu.intime, icu.outtime, asv.cvalue, asv.valuenum, asv.valueuom, 
  ...
  from mv_adults_sapsii_vars asv,
  icustays_adults icu
...
CREATE INDEX idx01_mv_adults_sapsii_vars_tw
  ON mimic3.mv_adults_sapsii_vars_tw
  USING btree
  (subject_id, hadm_id, itemcat COLLATE pg_catalog."default", itemid);
CREATE INDEX idx02_mv_adults_sapsii_vars_tw
  ON mimic3.mv_adults_sapsii_vars_tw
  USING btree
  (subject_id);
CREATE INDEX idx03_mv_adults_sapsii_vars_tw
  ON mimic3.mv_adults_sapsii_vars_tw
  USING btree
  (hadm_id);
  CREATE INDEX idx04_mv_adults_sapsii_vars_tw
  ON mimic3.mv_adults_sapsii_vars_tw
  USING btree
  (itemid);
  CREATE INDEX idx05_mv_adults_sapsii_vars_tw
  ON mimic3.mv_adults_sapsii_vars_tw
  USING btree
  (itemcat);

select count(*) from mv_adults_sapsii_vars_tw