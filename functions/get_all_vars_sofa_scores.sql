...
create or replace function get_all_vars_sofa_scores(icustay_id bigint, time_window integer)
	returns table (
	...
	cardiovascular_points double precision, respiration_points double precision,
	bilirubin_points double precision, creatinine_points double precision,
	urine_points double precision, platelets_points double precision,
    gcs_points double precision, total_sofa_score double precision
	) 
as $$
begin
	return query
	...
	select  pt_vars_scores.tw_hour_idx,
			...
			pt_vars_scores.weight,
                case 
					when locf(pt_vars_scores.cardiovascular_points) over (order by pt_vars_scores.tw_hour_idx) is null
						then 1
					else locf(pt_vars_scores.cardiovascular_points) over (order by pt_vars_scores.tw_hour_idx)
				end cardiovascular_points,
                case 
					when locf(pt_vars_scores.respiration_points) over (order by pt_vars_scores.tw_hour_idx) is null
						then 1
					else locf(pt_vars_scores.respiration_points) over (order by pt_vars_scores.tw_hour_idx)
				end respiration_points,
                case 
					when locf(pt_vars_scores.bilirubin_points) over (order by pt_vars_scores.tw_hour_idx) is null
						then 1
					else locf(pt_vars_scores.bilirubin_points) over (order by pt_vars_scores.tw_hour_idx)
				end bilirubin_points,
                case 
					when locf(pt_vars_scores.creatinine_points) over (order by pt_vars_scores.tw_hour_idx) is null
						then 1
					else locf(pt_vars_scores.creatinine_points) over (order by pt_vars_scores.tw_hour_idx)
				end creatinine_points,
                case 
					when locf(pt_vars_scores.urine_points) over (order by pt_vars_scores.tw_hour_idx) is null
						then 1
					else locf(pt_vars_scores.urine_points) over (order by pt_vars_scores.tw_hour_idx)
				end urine_points,
                case 
					when locf(pt_vars_scores.platelets_points) over (order by pt_vars_scores.tw_hour_idx) is null
						then 1
					else locf(pt_vars_scores.platelets_points) over (order by pt_vars_scores.tw_hour_idx)
				end platelets_points,
				case 
					when locf(pt_vars_scores.gcs_points) over (order by pt_vars_scores.tw_hour_idx) is null
						then 1
					else locf(pt_vars_scores.gcs_points) over (order by pt_vars_scores.tw_hour_idx)
				end gcs_points,
                case 
		...
		from (
		    ...
		    pt_vars.wbc,	
		    pt_vars.weight,
            pt_scores.cardiovascular_points, 
            pt_scores.respiration_points,
	        pt_scores.bilirubin_points, 
            pt_scores.creatinine_points,
	        pt_scores.urine_points, 
            pt_scores.platelets_points,
            pt_scores.gcs_points,
		    pt_scores.total_sofa_score
		    from mimic3.get_locf_pvt_all_vars($1,$2) as pt_vars
		    left join (
		    select pt_scores.tw_hour_idx, 
            pt_scores.cardiovascular_points, 
            pt_scores.respiration_points,
	        pt_scores.bilirubin_points, 
            pt_scores.creatinine_points,
            ...
            pt_scores.gcs_points,
            pt_scores.total_sofa_score
		    from mimic3.get_score_locf_pvt_sofa($1,$2) as pt_scores) as pt_scores
