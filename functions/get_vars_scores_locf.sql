...
select  pt_vars_scores.tw_hour_idx,
		...
		case 
			when locf(pt_vars_scores.total_apacheii_score) over (order by pt_vars_scores.tw_hour_idx) is null
				then 100000
			else locf(pt_vars_scores.total_apacheii_score) over (order by pt_vars_scores.tw_hour_idx)
		end total_apacheii_score
from (
