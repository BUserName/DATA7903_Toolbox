...
begin
	for icustay_id in (select distinct icu.icustay_id from icustays_adults icu order by icu.icustay_id) loop
		...
		insert into sparsity_all_vars(icustay_id, abpd, abpm, abps, nbpd, nbpm, nbps, papd, papm, paps, albumin, alt , arterial_ph , phosphorous , alkaline_phosphate , bilirubin , bun , calcium , calcium_ionized  , chloride , creatinine , fio2 , gcs , glucose , hco3 , hct , hemoglobin , hr , magnesium , mean_airway_pressure , o2_flow , paco2 , pao2 , platelets , potassium , ptt , rr , sao2 , sodium , spo2 , tco2 , temp , urine , wbc , weight)
		select icustay_id, sum(tmp.abpd)/ cast(row_cnt as double precision),
			sum(tmp.abpm)/ cast(row_cnt as double precision),
			sum(tmp.abps)/ cast(row_cnt as double precision),
			sum(tmp.nbpd)/ cast(row_cnt as double precision),
			sum(tmp.nbpm)/ cast(row_cnt as double precision),
			sum(tmp.nbps)/ cast(row_cnt as double precision),
			sum(tmp.papd)/ cast(row_cnt as double precision),
			sum(tmp.papm)/ cast(row_cnt as double precision),
			sum(tmp.paps)/ cast(row_cnt as double precision),
...
select * from sparsity_all_vars
delete from sparsity_all_vars
